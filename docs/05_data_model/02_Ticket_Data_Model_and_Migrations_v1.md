...
M1. Add ticket extension columns to `wp_pet_tickets`.
M2. Create `wp_pet_ticket_links`.
M3. Add `ticket_id` to `wp_pet_time_entries`.
M4. Add `ticket_id` to `wp_pet_tasks`.
M5. Backfill: create tickets for existing tasks; set tasks.ticket_id; optionally backfill time_entries.ticket_id via task_id → task.ticket_id.
M6. Add ticket_id to quote task records (draft linkage).
